<template>
  <div :class="['dialog__modal dialog__modal--second', {'dialog__modal--open' : $store.state.promptLoaderDialogStatus}]" data-e2e="prompt-loader">
    <div :class="['dialog__window dialog__window--prompt', {'dialog__window--open' : $store.state.promptLoaderDialogStatus}]">
          ...
          <div class="grid-unit--alpha">
            <form id="fileUploadForm" :class="['dropbox', {'dropbox--has-advanced-upload' : advancedDragAndDrop}]" method="post" action="" enctype="multipart/form-data">
              <div class="dropbox__input">
                ...
                <label for="file">
                  <strong><u>{{ $store.state.settings.dropbox.choosefilelabel }}</u></strong><br />
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      dialogClose() {
        this.$store.commit('SET_PROMPTLOADER_DIALOG', false);
      ...
      renameThePrompt(prompt) {
          IPCCCConfigurator.Request(this.customerId, 'prompts', 'save', prompt, prompt.Id).then(result => {
            this.promptData.prompt = result[0];
            if(this.promptData.Id != -1) {
              this.$store.commit('SET_PROMPTLOADER_DATA', {}); //DEEP COPY HACK
              this.$store.commit('SET_PROMPTLOADER_DATA', this.promptData);
            }
            this.dialogClose();
          }).catch(
          error => {
      ...
      saveThePrompt(prompt) {
          IPCCCConfigurator.Request(this.customerId, 'prompts', 'save', prompt, -1).then(result => {
            this.promptData.prompt = result[0];
            if(this.promptData.Id != -1) {
              this.$store.commit('SET_PROMPTLOADER_DATA', {}); //DEEP COPY HACK
              this.$store.commit('SET_PROMPTLOADER_DATA', this.promptData);
      ...
      checkAudioFile(audioFile) {
        ...
        if(_fileExtension !== 'undefined') {
          // _fileExtensionCheck = this.$store.getters.getCustomerInfo().customerId == 1 ? _fileExtension == 'mp3' || _fileExtension == 'wav' : _fileExtension == 'mp3';
      ...
      savePromptFileAndSaveRecord() {
        ...
        Axios.post(_fileName, this.newPromptAudioData).then(
          result => {
            this.promptData.prompt.PromptFiles[0].Location = result.data;
      ...
      validateForUpload() {
        if (this.actionEdit)
          this.okToSave =
            this.promptData.prompt.Title != '' &&
            this.promptData.promptNames.indexOf(this.promptData.prompt.Title) == -1;
        else {
          this.okToSave =
            this.promptData.prompt.Title != '' &&
            this.promptData.promptNames.indexOf(this.promptData.prompt.Title) == -1;
    ...
    mounted() {
      ...
      this.promptWatch = this.$store.watch(this.$store.getters.promptLoaderDialogStatus, status => {
        if(status) {
          this.resetData();
          this.promptData = this.$store.getters.promptLoaderData();
