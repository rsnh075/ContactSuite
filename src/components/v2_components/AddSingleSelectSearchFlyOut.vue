<template>
  <div class="add-singleselect-search__wrapper" data-e2e="v2-single-select-search-flyout">
    <button type="button" :class="['add-singleselect-search__add-btn', {'add-singleselect-search__add-btn--readonly' : addBtnReadOnlyCondition}]" @click="toggleList()" ref="addButton">&#xF419;</button>
		<div v-show="showList" :class="['add-singleselect-search__list-wrapper', {'add-singleselect-search__list-wrapper--show': showList}, arrowClass]" ref="listWrapper" id="add-singleselect-search__list-wrapper">
  	  <div class="add-singleselect-search__list-header">
  	    <span class="add-singleselect-search__list-title">{{ selectBoxConfig.title }}</span>
        <button type="button" class="add-singleselect-search__list-close-icon" @click="closeList()">&#xF156;</button>
  	  </div>
      <div class="add-singleselect-search__list-search">
      ...
      <div class="add-singleselect-search__list-body" ref="listBody">
        <ol>
          <li v-for="item in filteredItemList" @click="clickedItem(item)" class="add-singleselect-search__list-item">
            <span v-if="item.Icon" class="add-singleselect-search__list-item-icon" v-html="item.Icon"></span>{{ item[selectBoxConfig.filterProperty] }}
          </li>
        </ol>
      </div>
    </div>
...
<script>
  /**
   *
   * add button and selectbox flyout with search
   * closes all instances on select
   * returns selected item
   * label is the filterproperty and sits one level deep in the item
   *
   * selectBoxConfig : {
      filterProperty       : '',
      title                : ''
  ...
  export default {
    name: 'AddSingleSelectSearchFlyOut',
    props: {
      list : {
        type : Array,
        default : () => []
    ...
    methods: {
      ...
      toggleList() {
        [].slice.call(document.querySelectorAll('.add-singleselect-search__list-wrapper--show')).forEach(el => el.classList.remove('add-singleselect-search__list-wrapper--show'));
        this.positionFlyOut();
...
<style lang="scss">
  ...
  .add-singleselect-search__wrapper {
    display: block;
  }

  .add-singleselect-search__list-wrapper {
    display: none;
    position: fixed;
  ...
  .add-singleselect-search__list {
    ...
    &-item {
      ...
      &--selected, &--disabled {
        ...
        color: globals.$color-gray40;
        background-color: globals.$color-gray10;
        .add-singleselect-search__list-item-icon {
          color: globals.$color-gray40;
        }
      }
      &--disabled {
